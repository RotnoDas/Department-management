  ...
  const getLocation = () => {
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          setLocationError("Location access denied. Please enable location to mark attendance.");
        ...
        <div className="card-body p-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/90 mt-1">Mark your attendance for active classes</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="card-body">
          ...
          {activeSessions.length === 0 ? (
            <div className="text-center py-12">
              <div className="mb-4">
                <Library className="w-16 h-16 text-base-content/20 mx-auto" />
              </div>
                  ...
                  <div className="card-body p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h3 className="font-bold text-lg">{session.courseName}</h3>
                        <p className="text-sm text-base-content/60">{session.courseCode}</p>
                      </div>
                      {session.attendanceId && (
        ...
        <div className="card-body">
          ...
          {stats.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-base-content/60">No attendance data available yet</p>
            </div>
          ) : (
                    ...
                    <tr key={stat.courseCode}>
                      <td>
                        <div>
                          ...
                          <div className="text-sm text-base-content/60">{stat.courseCode}</div>
                        </div>
                      </td>
                      <td>{stat.totalSessions}</td>
                      <td>{stat.attendedSessions}</td>
                      <td>
                        <span className="badge badge-success badge-sm">{stat.presentCount}</span>
                      </td>
                      <td>
                        <span className="badge badge-warning badge-sm">{stat.lateCount}</span>
                      </td>
                      <td>
                        <div className="flex items-center gap-2">
                          <progress
